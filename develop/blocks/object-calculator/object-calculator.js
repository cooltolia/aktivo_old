;(function() {
    ...
    var chart = new Highcharts.chart('object_top_x_div', {
            chart: {
                type: 'column',
    ...
    var summSlider = $("input#object_summ1-range").rangeslider({
        ...
        yearSlider = $("input#object_summ2-range").rangeslider({
            polyfill: false,
            // Callback function
            onSlide: function (position, value) {
                update(2, value, true)
    ...
    var update = function (slider, val, onlyTextUpdate) {
        ...
        $("#object_summ2, #object_vyear").text(yearFormatter(years));

        $("#object_result1").text(abc2(total));
        $("#object_result1_percent").text("+" + percentFormatFunction(totalPercentage) + "%");

        $("#object_result2").text(abc2(totalRent));
        $("#object_result2_percent").text("+" + percentFormatFunction(totalRentPercent) + "%");

        $("#object_result3").text(abc2(totalCapitalization));
        $("#object_result3_percent").text("+" + percentFormatFunction(totalCapitalizationPercent) + "%");

        $("#object_result4").text(abc2(aktivoInvest));
        ...
        $('#object_summ1-scroll')
            ...
            .html('<div class="tooltip2">' +
                ...
                '</div>');
        if (income >= 1900000) {
            $("#object_summ1-scroll").addClass("no-right");
        } else {
            $("#object_summ1-scroll").removeClass("no-right");
        }
    };
    update();
    return {};
